...
/**
 * Webhook handler for target updates
 * Receives a list of IDs via POST and saves them to state
 * 
 * @param {object} params - Action parameters
 * @param {array} params.ids - List of target IDs to be updated
 * @returns {object} Result of the operation
 */
async function main(params) {
  const logger = Core.Logger('webhook-target-removes', { level: params.LOG_LEVEL || 'info' });
  ...
  try {
    logger.info('Received webhook request for target removes');
    ...
    const stateKey = `webhook-ids-removed${country}.${timestampMs}`;
    ...
    return {
      statusCode: 200,
      body: {
        message: 'Target remove request received',
